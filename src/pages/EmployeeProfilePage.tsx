...
function getEmpInvoices(emp: Employee, invoices: Invoice[], from?: string, to?: string) {
  return invoices.filter(inv => {
    const has = (inv.items || []).some(it => itemBelongsToEmployee(it, emp))
    if (!has) return false
...
export default function EmployeeProfilePage() {
  ...
  const empInvoices = getEmpInvoices(empNN, invoices, dateFrom || undefined, dateTo || undefined)
      ...
      <div className="proto-page-body">
        <div className="proto-kpi-grid-4" style={{ marginBottom: 14 }}>
          {[
            ...
            { label: 'Paid · lifetime', value: protoCurrency(totalPaid, 2), sub: `${paidCount} paid`, color: '#22c55e' },
            { label: 'Pending', value: protoCurrency(totalEarned - totalPaid, 2), sub: `${pendingCount} statements`, color: '#fb923c' },
          ].map(metric => (
        ...
        <div className="data-card" style={{ alignSelf: 'start' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(92px, 1fr))', gap: 8, marginBottom: 16 }}>
            ...
            <div className="settings-stat-card" style={{ borderColor: statementAdjustments.length > 0 ? 'var(--gold)' : undefined }}>
              <div className="settings-stat-count" style={{ fontSize: 15, color: statementAdjustments.length > 0 ? 'var(--gold)' : undefined }}>{formatMoney(netEarned)}</div>
          ...
          {empInvoices.length === 0 ? (
            ...
            <div>
                      ...
                      <table className="data-table">
                        ...
                        <tbody>
                          <tr>
                            ...
                            <td style={{ textAlign: 'right', color: 'var(--muted)', fontSize: 11 }}>{formatPayRateLabel(distinctPayRates(items, empNN))}</td>
              ...
              <div style={{ textAlign: 'right', fontSize: 13, padding: '8px 4px', borderTop: '2px solid var(--border)', marginTop: 4 }}>
                <div style={{ fontWeight: 700 }}>Gross &nbsp; {fmtHoursHM(totalHours)} &nbsp;&nbsp; {formatMoney(totalEarned)}</div>
